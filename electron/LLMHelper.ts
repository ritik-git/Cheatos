...
import { SYSTEM_PROMPT, PROMPTS } from "../shared/prompt"
import fs from "fs"
...
export class LLMHelper {
  ...
  public async extractProblemFromImages(imagePaths: string[]) {
    try {
      ...
      const prompt = PROMPTS.extractFromImages()
  ...
  public async generateSolution(problemInfo: any) {
    const prompt = PROMPTS.generateSolution(problemInfo)
  ...
  public async debugSolutionWithImages(problemInfo: any, currentCode: string, debugImagePaths: string[]) {
    try {
      ...
      const prompt = PROMPTS.debugWithImages(problemInfo, currentCode)
      const result = await this.model.generateContent([prompt, ...imageParts])
      const response = await result.response
      const text = this.cleanJsonResponse(response.text())
      const parsed = JSON.parse(text)
  ...
  public async analyzeAudioFile(audioPath: string) {
    try {
      ...
      const prompt = PROMPTS.analyzeAudio()
  ...
  public async analyzeAudioFromBase64(data: string, mimeType: string) {
    try {
      ...
      const prompt = PROMPTS.analyzeAudioQuick();
  ...
  public async analyzeImageFile(imagePath: string) {
    try {
      ...
      const prompt = PROMPTS.analyzeImage();
