...
class LLMHelper {
    ...
    async extractProblemFromImages(imagePaths) {
        try {
            ...
            const prompt = prompt_1.PROMPTS.extractFromImages();
    ...
    async generateSolution(problemInfo) {
        const prompt = prompt_1.PROMPTS.generateSolution(problemInfo);
    ...
    async debugSolutionWithImages(problemInfo, currentCode, debugImagePaths) {
        try {
            ...
            const prompt = prompt_1.PROMPTS.debugWithImages(problemInfo, currentCode);
            const result = await this.model.generateContent([prompt, ...imageParts]);
            const response = await result.response;
            const text = this.cleanJsonResponse(response.text());
            const parsed = JSON.parse(text);
    ...
    async analyzeAudioFile(audioPath) {
        try {
            ...
            const prompt = prompt_1.PROMPTS.analyzeAudio();
    ...
    async analyzeAudioFromBase64(data, mimeType) {
        try {
            ...
            const prompt = prompt_1.PROMPTS.analyzeAudioQuick();
    ...
    async analyzeImageFile(imagePath) {
        try {
            ...
            const prompt = prompt_1.PROMPTS.analyzeImage();
