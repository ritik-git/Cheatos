...
export const PROMPTS = {
  ...
  analyzeAudioQuick: (contextInput?: string): string => {
    const basePrompt = buildSystemPrompt(contextInput)
    // Optimized task description - more concise while maintaining clarity
    const task = `Task: Answer user questions quickly and naturally. Always respond when user finishes speaking.\n\nLanguage: English only. If other language detected, reply in English requesting English.\n\nFormat: Brief paragraph (2-3 sentences) + key points. Keep under 15 lines. No code/JSON/formatting.\n\nAlways respond to questions/statements. Sound human - casual, quick, natural.`
    return `${basePrompt}\n\n${task}`
  },
