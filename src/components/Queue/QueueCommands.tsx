      ...
      try {
        ...
        recorder.onstop = async () => {
          ...
          reader.onloadend = async () => {
            ...
            console.log('[UI] Calling analyzeAudioFromBase64 with blob type:', blob.type)
            try {
              const result = await window.electronAPI.analyzeAudioFromBase64(base64Data, blob.type)
              console.log('[UI] Received audio result:', result.text)
              setAudioResult(result.text)
            } catch (err) {
              console.error('[UI] Audio analysis error:', err)
        ...
        setAudioResult(`Could not start recording: ${err.message || 'Permission denied or microphone unavailable'}`)
      }
    } else {
      // Stop recording
      mediaRecorder?.stop()
